...
describe('workspaceBranchGuards cross-branch', () => {
  ...
  it('blocks quotation / customer / expense / incident / coil / associated staff from Yola on Kaduna', () => {
    const db = fakeDb([
      ...
      ['FROM associated_staff', { id: 'AS-YL', branch_id: 'BR-YL' }],
    ]);
    ...
    expect(assertAssociatedStaffIdInWorkspace(db, kadunaReq, 'AS-YL').ok).toBe(false);
  });
});
