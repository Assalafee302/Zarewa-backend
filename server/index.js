  ...
  const bootFixHint =
    lastBootPhase === 'migrations_failed' && /Could not acquire migration lock/i.test(errMsg)
      ...
      : lastBootPhase === 'migrations_failed' && /Atomics\.wait/i.test(errMsg)
        ? 'Boot migrations timed out talking to MySQL. Redeploy the latest backend, ensure only one API instance migrates at once, and set ZAREWA_MYSQL_SYNC_TIMEOUT_MS=900000 on the one-shot migrate/boot job (serving workers should keep ~10000). Run: npm run mysql:smoke'
